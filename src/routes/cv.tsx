    ...
    <ol className="relative space-y-8 border-l border-hairline pl-6">
      {items.map((it, i) => (
        ...
        >
          ...
          />
          <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">{it.period}</p>
  ...
  const isLandscape =
    typeof window !== "undefined"
      ? (document.querySelector(`img[src="${item.image}"]`) as HTMLImageElement)?.naturalWidth >
        (document.querySelector(`img[src="${item.image}"]`) as HTMLImageElement)?.naturalHeight
      : false;

  return (
    <div
      className={`overflow-hidden rounded-2xl border border-pink-strong/30 bg-card/80 shadow-[0_20px_50px_-15px_color-mix(in_oklab,var(--pink-strong)_60%,transparent)] backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200 ${isLandscape ? "max-w-[600px]" : "max-w-[440px]"}`}
    >
      <img
        src={item.image}
        alt={item.title}
